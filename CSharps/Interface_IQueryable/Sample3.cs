    ...
    public class Sample3
    {
        ...
        internal static void Demonstration()
        {
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                var dbset = new Query<StateInfos>(new DbQueryProvider(connection));

                Console.WriteLine(dbset.ex);

                string Code = "Free";

                var query = dbset.Select(s => new { Code = s.Code, DisplayName = s.DisplayName, Type = s.Type == 0 }).Where(s => s.Code == "Free");
        ...
        //            return (TResult)Activator.CreateInstance(typeof(ProjectionReader<>).MakeGenericType(elementType), BindingFlags.Instance | BindingFlags.NonPublic, null, new object[] { reader, projector }, null);
        //        }
        //        catch (Exception ex)
        //        {
        ...
        #endregion Improve Column Binding
    }
